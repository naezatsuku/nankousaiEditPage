...
const UsersTable = () => {
  ...
  const [updatingIds, setUpdatingIds] = useState<string[]>([])  // ← 追加
  ...
  const handleDeleteSelected = async () => {
    if (!window.confirm('ほんとにユーザーを削除しますか？')) return
  ...
  const fetchData = async () => {
    const { data: profiles, error } = await supabase
      .from('user_profiles')
      .select('*')
    if (error) return window.alert('データ取得エラー')
    ...
    <div className="w-full px-4 py-6 text-gray-700 overflow-x-auto">
      <div className="flex justify-center my-2">
        <button
          ...
          className="py-2 px-3 rounded-2xl bg-red-100"
        >
        ...
        <table className="w-full border-collapse rounded-t text-xs sm:text-sm">
          <thead className="bg-gray-100 text-left font-medium">
            <tr>
              ...
              <th className="px-3 py-2 hidden sm:table-cell">担当クラス</th>
          ...
          <tbody>
            {data.map(user => (
              ...
              >
                <td className="px-2 py-2 hidden sm:table-cell text-center">
                  <input
                    type="checkbox"
